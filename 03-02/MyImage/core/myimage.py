...
class ImageUtils:
        ...
        try:
            ...
            for file_name in os.listdir(self.source_dir):
                ...
                if os.path.isfile(image_path):
                    if fnmatch.fnmatch(file_name, '*.png') or  fnmatch.fnmatch(file_name, '*.jpg') or  fnmatch.fnmatch(file_name,'*.bmp') or fnmatch.fnmatch(file_name, '*.jpeg'):
    ...
    def rotate(self, file_name, point, transpose=False):
        ...
        if file_name in os.listdir(self.source_dir):
            ...
            im = Image.open(os.path.join(BASE_DIR, 'db', 'source', file_name))
            im_new = im.rotate(45)
            if transpose:
                im_new = im_new.transpose(Image.FLIP_LEFT_RIGHT)
            im_new.show()
            n1, n2 = file_name.split('.')
            im_new.save(os.path.join(BASE_DIR, 'db', 'target',n1+'-rotate'+'.'+n2))
    ...
    def cut(self, file_name, w1, h1 , w2, h2):
        ...
        if file_name in os.listdir(self.source_dir):
            ...
            im_new.show()
            n1, n2 = file_name.split('.')
            im_new.save(os.path.join(BASE_DIR, 'db', 'target',n1+'-crop'+'.'+n2))
